...
#time variables for STOPWATCH
t = time.time() # initialize time as current time
dt = 0 # initialize time difference as zero
time_unit = "seconds"
...
def webpage():
    ...
    <br>
    <p class="dotted">The time since you last pressed the STOPWATCH button is
    <span style="color: deeppink;"><span id="time">{dt}</span>°{time_unit}</span></p><br>
    </body>
    <script>
    // LED buttons handler
...
def buttonpress(request: HTTPRequest):
    ...
    if "STOPWATCH" in query:
        ...
        t = end_time # update current time
    # Acknowledge
    with HTTPResponse(request, content_type=MIMEType.TYPE_HTML) as response:
        response.send()

...
void ws2812b_setup() {
    T2CONbits.TCKPS = 0; // TCKPS = 0 for a Timer2 prescaler of N=1 (1:1)
    PR2 = 65535; // maximum period = (PR2+1) * N * 1/48MHz
    TMR2 = 0; // initialize Timer2 to 0
    T2CONbits.ON = 1; // turn on Timer2

    // initialize output pin as off
    TRISBbits.TRISB6 = 0; //pin B6 is an output pin
    LATBbits.LATB6 = 0; // pin B6 is OFF
}
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    // loop through each WS2812B to pre-calculate the time the pin needs to toggle
    for (i = 0; i < numLEDs; i++) {
        // loop through each color bit, MSB first
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].r>>j)&0b1) { //??????????????????????? if the bit is a 1
                // the high is longer
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].g>>j)&0b1) { //??????????????????????? if the bit is a 1
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].b>>j)&0b1) { //??????????????????????? if the bit is a 1
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
...
// hue is a number from 0 to 360 that describes a color on the color wheel (0 deg on color wheel = red, 180 deg = red, etc)
...
wsColor HSBtoRGB(float hue, float sat, float brightness) { //output is an RGB number
    float red = 0.0;
    float green = 0.0;
    float blue = 0.0;
